...
export class AnimalsApi {
  ...
  static async getUserBookmarks(id: number){
    return ApiClient.get(AnimalsRouts.ANIMAL_BOOKMARKS(id), {
      __tokenRequired: true
    })
  }
}
...
export class PostsApi {
  ...
  static async getComments(PostId: number, content_type: string) {
    try {
      const response = await ApiClient.get(CommonRouts.COMMENTS_ID(PostId, content_type), {
        __tokenRequired: false,
      });
      return response.data;
    } catch (error) {
